...
/* Windowed DFT for HATS using FFTW. Based on Tiago Giorgetti's implementation in Matlab, "Influência das janelas de ponderação na 
obtenção da amplitude de sinais periódicos no domínio da frequência".
...
void windowed_dft(double *signal, complex *fft_out, double *windowed_signal, double *mod, double *arg, int window_type, int saved_files, int signal_lenght){
	...
	switch(window_type){
		...
		case 1:
			...
			break;
		/*case 2:
			//Is way different. This is defined in terms of the frequency response. We need to calculate the inverse dft and of an constant signal(?) then normalize it by its max norm and then apply it to the signal. Weird......
...
/*Description:
	Returns only the amplitude of <target_frequency> of the <signal> array using the goertzel algorithm 
Recieves:
	<signal> pointer to complex: is an array with the dft of the signal with lenght <signal_lenght>
	<target_frequency> pointer to double: is an array with lenght floor(<signal_lenght>/2)+1. (This is due to periodic properties of the real valued dft)
	...
	<window_type> int: defines the type of filter to be applyied to cut the signal
		...
		-> 4, Triangular
Returns:
	Double with the amplitude of the <target_frequency_k> frequency*/
...
double goertzel_amplitude(double *signal, double target_frequency, int signal_lenght, int sampling_frequency, int window_type){
	...
	//is the approximate frequency that can be calculated
	const int target_frequency_k = (int) floor(target_frequency*signal_lenght/sampling_frequency); 	
...
/*Description: 
	Calculates the modulus or amplitude of an array of complex numbers.
Recieves:
...
/*Description: 
	Calculates the phase or principal argument of an array of complex numbers. Returns an array of real numbers.
Recieves:
	<in> pointer to complex: is an array with the dft of the signal with lenght <signal_lenght>
	<out> pointer to double: is an array with lenght floor(<signal_lenght>/2)+1. (This is due to periodic properties of the real valued dft)
	<signal_lenght> int: number of data points in the signal
...
/*Description: 
	Applyies the rectangular filter to the signal.
Recieves:
...
void rectangularwin(double *in, double *out, int signal_lenght){
	int i;
	const double correction_param = 2; //multiplying by the amplitude calibration coeficient. This makes sure that the sinusoidal DFT windowed by this will be accurate in its amplitude 
	for(i=0; i<signal_lenght; i++){
		out[i] = correction_param*in[i]; 
	}
	return;
}

/*Description: 
	Applyies the Symmetric Flat top window to the signal. Taken from "type 0", ISO 18431-1 (Chique, né?)
...
void flattopwin(double *in, double *out, int signal_lenght){
	int i;
	double weight;
	const double correction_param = 2.000122419602196; //with the corrected coeficient by amplitude. Differs from Tiago Giorgetti and I don't know why.
	for (i=0; i<signal_lenght;i++){
		weight = 1.0 - 1.9330*cos(2*M_PI*i/(signal_lenght-1)) + 1.2860*cos(4*M_PI*i/(signal_lenght-1))-0.388*cos(6*M_PI*i/(signal_lenght-1))+0.0322*cos(8*M_PI*i/(signal_lenght-1));
...
/*Description: 
	Applyies the Hanning window to the signal. Taken from Lizhe Tan and Jean Jiang, "Chapter 4 - Discrete Fourier Transform and 
	Signal Spectrum" and from https://www.dadisp.com/webhelp/mergedProjects/refman2/FncrefFK/HANNING.htm
Recieves:
	<in> pointer to double: is the array with signal of lenght <signal_lenght>
	<out> pointer to double: is the array after applying the filter with lenght <signal_lenght>.
	<signal_lenght> int: number of data points in the signal
...
/*Description: 
	Applyies the Triangular window to the signal. Taken from Lizhe Tan and Jean Jiang, "Chapter 4 - Discrete Fourier Transform and 
	Signal Spectrum"
Recieves:
	<in> pointer to double: is the array with signal of lenght <signal_lenght>
	<out> pointer to double: is the array after applying the filter with lenght <signal_lenght>.
...
void triangwin(double *in, double *out, int signal_lenght){
	int i;
	double weight;
	const double correction_param = 4.000244170177297; //This coeficient makes sure that a "pure" sinusoidal will have its amplitude preserved
	for(i = 0; i<signal_lenght; i++){
