...
/*
	...
	---------------------------------------------------------------------------------	
	  0.3	|  13-10-2021   | Atualizacao, ativando o opmode_var->object para SKY no
	  	|		| inicio da execucao.
	---------------------------------------------------------------------------------
	  0.4   |  07-09-2024   | Implementacao de az_offset como parametro de entrada.
	_________________________________________________________________________________
...
//Opt Arguments
char *azoffset_value	= "5"	;	// Default Parameter for az_offset = 5 degrees
...
int main(int argc , char *argv[])
{
	...
	while (1)
	{
		static struct option long_options[] =
		{
			...
			{"az_offset", required_argument,0,	0},
		...
		switch (c)
		{
			case 0:
				...
				if (long_options[option_index].flag != 0)
					break;
				 if (memcmp( long_options[option_index].name,"az_offset",9)==0)
                                {
                                        azoffset_flag = 1;
                                        optflag_ctr++;
                                        printf("%s_flag: ", long_options[option_index].name);
                                        if (optarg)
                                        azoffset_value = optarg;
                                                printf("%s\n",azoffset_value); 
                                }
	...
	if (debug_flag)
	{
		...
		if(azoffset_flag)	printf("azoffset_flag = %d\n", azoffset_flag);
...
*/
	...
	int azoffset_dvalue			;	//Decimal value for azoffset_value opt parameter
	...
	azoffset_dvalue = atoi(azoffset_value);
        if (debug_flag) printf("azoffset_dvalue = %d\n", azoffset_dvalue);
	...
	for (el = elinit_dvalue ; el < elend_dvalue+1 ; el += step_dvalue)
	{
		...
		sprintf(skyDip_data[11],"	var az = sky6RASCOMTele.dAz+%d;			",azoffset_dvalue);
		...
		if (el == elinit_dvalue)
		{
			if (debug_flag) printf("\nSlewing to SkyDip position...Az offset = %d degrees\n\n",azoffset_dvalue);
			if (verbose_flag) printf("\nSlewing to SkyDip position...Az offset = %d degrees\n\n",azoffset_dvalue);
		}else 
		{	if (debug_flag) printf("Slewing to next SkyDip position...Az offset = %d degrees\n",azoffset_dvalue);
			if (verbose_flag) printf("\rSlewing to next SkyDip position...Az offset = %d degrees\n",azoffset_dvalue);
		}
