/*
	...
	---------------------------------------------------------------------------------
		...
		|		| utilização da função memcmp() para comparar strings.
	        |		| Atualização do pTrack.h, edição do usage.	
	---------------------------------------------------------------------------------
...
*/
	...
	if ( memcmp(server_reply,"TypeError: The operation failed because the mount is not yet homed. Error = 231.|No error. Error = 0.", strlen(server_reply)) == 0 )
	{
		...
		if (ch != 'y')
		{
			...
			printf("Finding home position data instructions sent to remote server.\n");
			...
			printf("Find home position data received from socket server.\n");
			...
			if ( memcmp(server_reply , "Homed|No error. Error = 0." , strlen(server_reply)) != 0 )   //Find home complete  
			{
				...
				log_info("Resuming the slewing to %s now...\n", argv[1]);
				...
				// - - - - - - - - - - - - - - - - - - - - - - - - - - -
				if( memcmp(server_reply , "isSlew=1" , strlen(server_reply)) != 0 )     // "1" = Slew completed
				{
					log_info("Slew to %s completed. Tracking on %s RaDec rates.", argv[1], argv[1]);
					printf("Slew to %s completed. Tracking on %s RaDec rates.\n\n", argv[1], argv[1]);
					...
					printf("The condition isSlew=1 was not matched.\n");
				}
			}else{
				printf("The condition HOMED was not matched.\n");
			}
		}
	}
