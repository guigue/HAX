/*
        ...
        Delete Shared Memories - version 0.2
	--------------------------------------------------------------------------------
	...
	--------------------------------------------------------------------------------
        ...
        --------------------------------------------------------------------------------
	  0.2	|  29-11-2021	| Atualizacao nas linhas 306 adiante, incluindo 
	  	|		| mensagens para informar as memorias removidas.
 	________________________________________________________________________________
...
int main(int argc , char *argv[])
{
	...
	printf("Config_BackingFile:'%s' .......deleted.\n",BackingFileIN);
	sem_unlink(SemaphoreNameIN);
	printf("Config_SemaphoreFile:'%s' .......deleted.\n\n",SemaphoreNameIN);

	shm_unlink(OpMode_BackingFile);
	printf("OpMode_BackingFile:'%s' .......deleted.\n",OpMode_BackingFile);
	sem_unlink(OpMode_SemaphoreName);
	printf("OpMode_SemaphoreFile:'%s' .......deleted.\n\n",OpMode_SemaphoreName);
	 
	shm_unlink(GetPos_BackingFile);
	printf("GetPos_BackingFile:'%s' .......deleted.\n",GetPos_BackingFile);
	sem_unlink(GetPos_SemaphoreName);
	printf("GetPos_SemaphoreFile:'%s' .......deleted.\n\n",GetPos_SemaphoreName);
