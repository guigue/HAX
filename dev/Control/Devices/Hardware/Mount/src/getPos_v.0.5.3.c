...
/*
	...
	--------------------------------------------------------------------------------
	  0.6	|  		| Adequacao para utilizaçao do codigo cfgCatcher para 
	...
	*****************************************************************/
	...
	while(!kbhit())
	{
		...
		if ((rb_ctr%RINGSIZE) == 0)
		{
			...
			begin = end;
			if (debug_flag) printf("TEMPO = %20.10f | CLOCKS_PER_SEC = %ld | Begin = %20.10f | End = %20.10f \n",(double)Dtime,CLOCKS_PER_SEC,(double)begin,(double)end);
