...
/*
	...
	parado com o tracking desativado, e com a marca op_mode=STALL.
...
#include "skyDip.h"	//Definitions for pTrack
...
*/
	...
	int el 					;	//Elevation Coordinate, starts with 10 degree
	...
	for (el = 10 ; el < 91 ; el += 10)
	{
		...
		}else if ( memcmp(server_reply,"SkyDip.|No error. Error = 0.", strlen(server_reply)) == 0 )
		{
			...
			if (verbose_flag)  printf("\rWaiting 10 seconds...\n\n");
			sleep(10);  //Must be 10 seconds
		...
		{
			if (debug_flag || verbose_flag) printf("Server reply not understandable\n");
