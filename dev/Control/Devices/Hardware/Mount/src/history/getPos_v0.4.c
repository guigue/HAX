/*
	...
	GetPositionLoop_v0.3
	---------------------------------------------------------------------------------
	Fazendo o uso de memória compartilhada e ring buffer, faz solicitações javascript
	ao servidor TheSkyX em loop infinito em busca de indicadores de status do
	posicionamento do mount do telescópio. É gerado um arquivo binário contendo:
	tempo Husec, tempo Julian date, tempo Sideral, posições de Azimute, Elevação,
	Ascenção Reta e Declinação, e as velocidades de tracking para Ascenção Reta e
	 Declinação, como a diferença para a taxa sideral.

	Foi implementado também um sistema de logging e recurso para utilização de
	arquivo de configuração texto para acesso de parâmetros modificáveis pelo
	programa executável.
	...
	________|_______________|________________________________________________________
...
int main(int argc , char *argv[])
{
        ...
        static char *BackingFile;
        static char *SemaphoreName;
        static long int AccessPerms;

        static long int RINGSIZE;
        ...
        cfg = cfg_init(opts, 0);
        if( cfg_parse(cfg, "/opt/HAX/Control/Devices/Hardware/Mount/etc/HAX_Control.config") == CFG_FILE_ERROR)
        {
...
uint64_t husec_time(void)
{
        ...
        static long int SEC2HUSEC;
        static long int HUSEC2NSEC;
        static long int ONEDAY;
        static long int HUSECS2HRS;
        static long int MIN2HUSEC;
        static long int MIN;
