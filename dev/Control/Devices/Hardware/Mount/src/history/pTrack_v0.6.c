/*
	...
	---------------------------------------------------------------------------------
		...
		|  09-08-2021	| Melhoria no tratamento das respostas do servidor com a
...
//	char * p				;	// Pointer for manage server reply
//	const char sep[2] = ";"			;	// Especial caracter to manage server reply
	char  chk_object_data[10][50]		;	// Instructions to check if object exists in the server.
	int chk_object_data_nlines = 10		;	// Getter commands number of lines from chk_object_data[].
	...
	sprintf(chk_object_data[0],"/* Java Script */									");
	sprintf(chk_object_data[1],"/* Socket Start Packet */								");
	sprintf(chk_object_data[2],"var Out;										");
	sprintf(chk_object_data[3],"var err;										");
	sprintf(chk_object_data[4],"var obj='%s';									",argv[1]);
	sprintf(chk_object_data[5],"sky6StarChart.LASTCOMERROR = 0;							");
	sprintf(chk_object_data[6],"sky6StarChart.Find(obj);								");
	sprintf(chk_object_data[7],"err = sky6StarChart.LASTCOMERROR;							");
	sprintf(chk_object_data[8],"Out = err;										");
	sprintf(chk_object_data[9],"/* Socket End Packet */								");
...
//	int res;
	...
	{
		//res = strcmp(argv[1],keywordmap[i].keyword);
	...
	if (keywordmap[i].targets == EXTRA_OBJ)
	{
		...
		if (ch != 'c')
		{
			log_error("Target is an extra object. Aborted by user");
	...
	for(i = 0; i<chk_object_data_nlines ; i++)
        {
		if( send(sock , chk_object_data[i] , strlen(chk_object_data[i]) , 0) < 0)
		{
	...
	//****************************************************
	int reply_cut_ctr = count_char(server_reply, '|');	//Count a string with a char limiter	
	char reply_cut1[reply_cut_ctr];				//Creat a fixed string
	copy_char ( server_reply, reply_cut1,'|');		//Copy a string with a char limiter
	
	if ( memcmp(reply_cut1,"TypeError: Object not found. Error = 250.", reply_cut_ctr) == 0 )        
	{
		...
		printf("\r\nTarget to %s selected. Slewing...\n", argv[1]);
		log_info("Target to %s selected. Slewing...", argv[1]);
	...
	//REPLY DATA ANALYZER 2 
	//*********************
	reply_cut_ctr = count_char(server_reply, '|');		//Count a string with a char limiter
	char reply_cut2[reply_cut_ctr];				//Creat a fixed string
	copy_char ( server_reply, reply_cut2,'|');		//Copy a string with a char limiter
        ...
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        if ( memcmp(reply_cut2,"TypeError: Error, the device is parked and must be unparked using the Unpark command before proceeding. Error = 216.",reply_cut_ctr) == 0 )
        {
                log_error("Slew Aborted! Telescope is Parked. Try typing -u to UnPark.");
                printf("\nSlew to %s aborted. Telescope is Parked. Try typing -u to UnPark.\n\n", argv[1]);
                report_and_exit("Normal exit");
        }

        // Is the telescope found some problems to do the slew? LIMITS EXCEEDED
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        if ( memcmp(reply_cut2,"TypeError: Limits exceeded. Error = 218.",reply_cut_ctr) == 0 )
        {
                log_error("Slew Aborted! Limits Exceeded.");
                printf("\nSlew to %s aborted. Limits Exceeded.\n\n", argv[1]);
                report_and_exit("Normal exit");
        }
	...
	//VERIFYING IF SLEW IS COMPLETE
	while ( memcmp(reply_cut2,"isSlew=1",reply_cut_ctr) != 0 )     // "1" = Slew completed
	{	
		if( recv(sock , server_reply , RCV_BUFFER_SIZE , 0) < 0)
        	{
			log_error("Could not receive data from socket server.");
