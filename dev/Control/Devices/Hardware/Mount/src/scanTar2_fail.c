...
void StepSlewScan_a( char * object,
		      	int scanType_dvalue,
			double ra,
			double dec,
			double az,
			...
			sem_t * semptr_opmode,
			opmode_data_type * opmode_var,
			size_t ByteSize_opmode,
			int fd_shmem_opmode,
			FILE * fd,
			static char IP_SERVER[csize],
        		static long int TCP_PORT,
        		static long int RCV_BUFFER_SIZE,
        		static long int TX_DELAY);
void StepSlewScan_b( char * object,
		      	int scanType_dvalue,
			double ra,
			double dec,
			double az,
...
*/
	...
	log_info("-------------------------------------------\n");
	log_info("Object = %s\n", object);
	log_info("Nscan  = %d times\n", nscan_dvalue);
	log_info("Offset = %f [arcmin]\n", offsetScan_dvalue);
	log_info("Step	 = %f [arcmin]\n", stepSlew_dvalue);
	log_info("Type   = %d [type: RA=1, Dec=2, Az=3, El=4]\n", scanType_dvalue);
	log_info("-------------------------------------------\n");
	...
	double offset1_ra = ra + offset;
	double offset2_ra = ra - offset;
	double offset1_dec = dec + offset;
	double offset2_dec = dec - offset;
	double offset1_az = az + offset;
	double offset2_az = az - offset;
	double offset1_el = el + offset;
	double offset2_el = el - offset;
	...
	//---------------------------------------------------------------------------------------------
	char scanTar_data_2[20][150]          ;	// Data to send to server
	int scanTar_data_2_nlines = 20        ;	// Getter commands number of lines from scanTar_data[].
	
	sprintf(scanTar_data_2[0],"/* Java Script */ ");
	sprintf(scanTar_data_2[1],"/* Socket Start Packet */ ");
	sprintf(scanTar_data_2[2],"var Out; ");
	...
	sprintf(scanTar_data_2[13],"if (scanType == 1) {sky6RASCOMTele.SlewToRaDec(offset1_ra, dec, object);} ");
	sprintf(scanTar_data_2[14],"if (scanType == 2) {sky6RASCOMTele.SlewToRaDec(ra, offset1_dec, object);} ");
	sprintf(scanTar_data_2[15],"if (scanType == 3) {sky6RASCOMTele.SlewToAzAlt(offset1_az, el, object); } ");
	sprintf(scanTar_data_2[16],"if (scanType == 4) {sky6RASCOMTele.SlewToAzAlt(az, offset1_el, object); } ");
	sprintf(scanTar_data_2[17],"while(SlewComplete != 1){ ");
	sprintf(scanTar_data_2[18],"         SlewComplete = sky6Web.IsSlewComplete;} ");
	sprintf(scanTar_data_2[19],"Out = 'OffsetDone.'; ");
	sprintf(scanTar_data_2[20],"/* Socket End Packet */ ");
	...
	{
		if (verbose_flag) printf("Offset Position Done. Ready to start the Scan.\n\n");
	...
	sprintf(scanTar_data_5[2],"var Out; ");
		      ...
		      static long int TX_DELAY )
{
	...
	//---------------------------------------------------------------------------------------------
	char scanTar_data_3[26][150]          ;	// Data to send to server
	int scanTar_data_3_nlines = 26        ;	// Getter commands number of lines from scanTar_data[].
	
	sprintf(scanTar_data_3[0],"/* Java Script */ ");
	sprintf(scanTar_data_3[1],"/* Socket Start Packet */ ");
	sprintf(scanTar_data_3[2],"var Out; ");
	...
	sprintf(scanTar_data_3[13],"if (scanType == 1) {sky6RASCOMTele.SlewToRaDec(offset1_ra, dec, object);} ");
	sprintf(scanTar_data_3[14],"if (scanType == 2) {sky6RASCOMTele.SlewToRaDec(ra, offset1_dec, object);} ");
	sprintf(scanTar_data_3[15],"if (scanType == 3) {sky6StarChart.Find(object);                           ");
	sprintf(scanTar_data_3[16],"			sky6ObjectInformation.Property(58);                   ");
	sprintf(scanTar_data_3[17],"			az = sky6ObjectInformation.ObjInfoPropOut;	      ");
	sprintf(scanTar_data_3[18],"			sky6RASCOMTele.SlewToAzAlt(offset1_az, el, object); } ");
	sprintf(scanTar_data_3[19],"if (scanType == 4) {sky6StarChart.Find(object);			      ");
	sprintf(scanTar_data_3[20],"			sky6ObjectInformation.Property(59);		      ");
	sprintf(scanTar_data_3[21],"			el = sky6ObjectInformation.ObjInfoPropOut;	      ");
	sprintf(scanTar_data_3[22],"			sky6RASCOMTele.SlewToAzAlt(az, offset1_el, object); } ");
	sprintf(scanTar_data_3[23],"while(SlewComplete != 1){ ");
	sprintf(scanTar_data_3[24],"         SlewComplete = sky6Web.IsSlewComplete;} ");
	sprintf(scanTar_data_3[25],"Out = 'StepSlewDone.'; ");
	sprintf(scanTar_data_3[26],"/* Socket End Packet */ ");
		      ...
		      static long int TX_DELAY )
{
	...
	//Rotina Javascript para realizacao do SCAN - Step-by-Step Slew.
	//---------------------------------------------------------------------------------------------
	char scanTar_data_4[26][150]          ;	// Data to send to server
	int scanTar_data_4_nlines = 26        ;	// Getter commands number of lines from scanTar_data[].
	
	sprintf(scanTar_data_4[0],"/* Java Script */ ");
	sprintf(scanTar_data_4[1],"/* Socket Start Packet */ ");
	sprintf(scanTar_data_4[2],"var Out; ");
	...
	sprintf(scanTar_data_4[13],"if (scanType == 1) {sky6RASCOMTele.SlewToRaDec(offset2_ra, dec, object);} ");
	sprintf(scanTar_data_4[14],"if (scanType == 2) {sky6RASCOMTele.SlewToRaDec(ra, offset2_dec, object);} ");
	sprintf(scanTar_data_4[15],"if (scanType == 3) {sky6StarChart.Find(object);                           ");
	sprintf(scanTar_data_4[16],"			sky6ObjectInformation.Property(58);                   ");
	sprintf(scanTar_data_4[17],"			az = sky6ObjectInformation.ObjInfoPropOut;	      ");
	sprintf(scanTar_data_4[18],"			sky6RASCOMTele.SlewToAzAlt(offset2_az, el, object); } ");
	sprintf(scanTar_data_4[19],"if (scanType == 4) {sky6StarChart.Find(object);			      ");
	sprintf(scanTar_data_4[20],"			sky6ObjectInformation.Property(59);		      ");
	sprintf(scanTar_data_4[21],"			el = sky6ObjectInformation.ObjInfoPropOut;	      ");
	sprintf(scanTar_data_4[22],"			sky6RASCOMTele.SlewToAzAlt(az, offset2_el, object); } ");
	sprintf(scanTar_data_4[23],"while(SlewComplete != 1){ ");
	sprintf(scanTar_data_4[24],"         SlewComplete = sky6Web.IsSlewComplete;} ");
	sprintf(scanTar_data_4[25],"Out = 'StepSlewDone.'; ");
	sprintf(scanTar_data_4[26],"/* Socket End Packet */ ");
