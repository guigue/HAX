...
*/
	...
	int nscan_dvalue			;
	int arcminScan_dvalue			;
	int offsetScan_dvalue			;
	...
	arcminScan_dvalue = atoi(arcminScan_value);
	if (debug_flag) printf("arcminScan_dvalue = %d\n", arcminScan_dvalue);

	offsetScan_dvalue = atoi(offsetScan_value);
	if (debug_flag) printf("offsetScan_dvalue = %d\n", offsetScan_dvalue);
	...
	if( nscan_dvalue == 1 )
	{
		...
		if (debug_flag) printf("\nPerforming one scan of %s, please wait...\n\n", keywordmap[j].keyword);
		if (verbose_flag) printf("\nPerforming one scan of %s, please wait...\n", keywordmap[j].keyword);
	}else
	{
		if (debug_flag) printf("\nPerforming scan of %s %d times, please wait...\n\n", keywordmap[j].keyword, nscan_dvalue);
		if (verbose_flag) printf("\nPerforming scan of %s %d times, please wait...\n", keywordmap[j].keyword, nscan_dvalue);
	}
	...
	}else if ( memcmp(server_reply,"ScanTar.Done.|No error. Error = 0.", strlen(server_reply)) == 0 )
	{
		...
		log_info("Scan Target Done. Telescope is back to %s. Tracking...", keywordmap[j].keyword);
		if (debug_flag || verbose_flag)  printf("\rScan Target Done. Telescope is back to %s. Tracking...\n", keywordmap[j].keyword);
