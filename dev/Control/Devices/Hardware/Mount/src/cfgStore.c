/*
	...
	Configuration Store - version 0.7
	--------------------------------------------------------------------------------
	...
	--------------------------------------------------------------------------------
	  0.3	|  05-04-2021	| Pequenas modificacoes para organizar nomenclatura de
        ...
        --------------------------------------------------------------------------------
	  0.4	|  14-10-2021	| Inclusao do campo DATA_DIR para implementacao de 
  		|		| gravacao de arquivo rotativo para getPos.
        --------------------------------------------------------------------------------
	  0.5	|  20-10-2021	| Inclusao do campo WRITEBLK para implementacao de novo
  		|		| algoritmo para RingBuffer, para gravacao em arquivos
		|		| a cada bloco de registros (Write Block).
      	--------------------------------------------------------------------------------
	  0.6	|  21-10-2021	| Inclusao do campo GETPOS_INTERVAL para implementacao
  		|		| do recurso de timming, para controlar o tempo entre as
		|		| conexoes com o servidor TheSkyX em busca de
		|		| informacoes para compor o status via getPos.	  
	--------------------------------------------------------------------------------
	  0.7	|  29-10-2021	| Inclusao do campo SDTDPID para implementacao do 
	  	|		| recurso de DAEMON para o getPos. O campo insere o 
		|		| caminho para o arquivo onde estara armazenado o PID.
...
int main(int argc , char *argv[])
{
	...
	static char *GetPos_BackingFile			= NULL		;
        ...
        static long int WRITEBLK			= 0		;
	static long int GETPOS_INTERVAL			= 0		;
	...
	//--------------------- via confuse.h ------------------------------
		...
		CFG_SIMPLE_STR ("DAEMON_PID", &SDTDPID),
		...
		CFG_SIMPLE_INT ("WRITEBLK", &WRITEBLK),
		CFG_SIMPLE_INT ("GETPOS_INTERVAL", &GETPOS_INTERVAL),
                CFG_SIMPLE_STR ("DATAFILENAME", &DATAFILENAME),
		CFG_SIMPLE_STR ("DATA_DIR", &DATA_DIR),
        ...
        if( cfg_parse(cfg, "/opt/HAX/etc/HAX.config") == CFG_FILE_ERROR)
  	{
	...
	strncpy(config_var->SDTDPID,SDTDPID,csize)				;
	...
	strncpy(config_var->DATA_DIR,DATA_DIR,csize)				;
	...
	config_var->WRITEBLK			= WRITEBLK			;
	config_var->GETPOS_INTERVAL		= GETPOS_INTERVAL		;
