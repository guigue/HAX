...
/*
	...
	-------------------------------------------------------------------------------
	  0.6.6 |  26-11-2021	| Mudancas em alguns textos em mensagens de log RDB para
	  	|		| AUX, na parte de gravacao de arquivos .aux
	--------------------------------------------------------------------------------
	  0.6.7 |  29-11-2021	| Correcao de bug: Falha quando executa getPos sem 
	  	|		| argumentos. Correcao feita na linha 390.
	--------------------------------------------------------------------------------
	  0.6.8 |  02-12-2021	| Implementacao da funcao de STOP, para executar Kill 
	  	|		| no processo getPos --daemon.
...
int main(int argc , char *argv[])
{
	...
	while (1)
	{
		...
		c = getopt_long (argc, argv, "vhcds", long_options, &option_index);
	...
	if (argc < 2)
	{
		printf("getPos: There is no options, please try -h or --help.\n\n");
		exit(1);
	}
...
//	sprintf(get_data[16],"/* Socket End Packet */");
	...
	{
		report_and_exit("Can't increment semaphore to permit read opmode data.");
	}
...
*/
	...
	if (fd_shmem < 0)
	{
		report_and_exit("Can't open ring buffer shared mem segment...");
		log_error("Can't open ring buffer shared mem segment...");
	}
...
void save_data ( int fd_data, pos_data_type * wbpos_ptr, size_t WB_ByteSize)
{
	...
	{
		char msg[80] ;
		sprintf( msg , "Writing AUX file failed.  Error = %d", errno );
		log_fatal("Calling process was killed. When trying to write AUX file. Error = %d", errno);
		close_file ();
		exit(1);
	}
}
...
//	char hats_file_name[120];	//---> Set as global variable
	...
	{
		char msg[160] ;
		sprintf( msg ,"Open AUX file %s failed. Error = %s", hats_full_file_name,strerror(errno)); 
		log_fatal("Calling process was killed. When trying to open AUX file: %s Error = %s", hats_full_file_name,strerror(errno));
		exit(1);
	} else 
	{
		sprintf( msg ,"Opened AUX file  %s", hats_file_name); 
	//	log_info("Opened AUX file: %s", hats_file_name);
	}
}
...
void close_file (void)
{
	char msg[120];
	close(fd_data);
	sprintf( msg, "Closed AUX file %s", hats_file_name) ;  
	//log_info("Closed AUX file: %s", hats_file_name);  
}
